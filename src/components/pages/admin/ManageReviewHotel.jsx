...
export default function ManageReviewHotel() {
    ...
    const fetchReviews = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          `${import.meta.env.VITE_API_URL}/admin/list-review-hotel`,
          {
            headers: {
    ...
    if (searchTerm) {
      setFilteredReviews(
        reviews.filter((review) =>
          review.Hotels.name.toLowerCase().includes(searchTerm.toLowerCase())
        )
      );
    } else {
  ...
  const handleDeleteReview = async (reviewId) => {
    ...
    try {
      await axiosInstance.delete(
        `${import.meta.env.VITE_API_URL}/admin/reviews-hotel/${reviewId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleUpdateReview = async () => {
    ...
    try {
      await axiosInstance.post(
        `${import.meta.env.VITE_API_URL}/admin/update-review-hotel`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
