...
export default function TourDetail({ loading, tourDetail }) {
  if (loading) {
  ...
  return (
    <>
      <div className="flex flex-col gap-4">
        <h1 className="text-2xl font-semibold">Tour Itinerary</h1>
        <div className="flex flex-col gap-4">
          <div className="font-semibold flex items-center justify-between">
            <p>{tourDetail?.name}</p>
            <IconArrowDown />
          </div>
          <p className="text-[#4B5563]">{tourDetail?.description}</p>
          <div className="w-full flex items-center gap-4">
            <img className="rounded-xl" src={tourDetail.image_url} alt="tour" />
          </div>
        </div>
      </div>
      <div className="flex flex-col gap-4">
